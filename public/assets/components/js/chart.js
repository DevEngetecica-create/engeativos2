...
$(function () {
  ...
  const datas = arrayAtivosExternos.map(row => row.mes)

  // Função para formatar uma data no formato 'Mês Ano' com mês abreviado e hífen
  ...
  const datasFormatadas = datas.map(data => formatarData(data));

  var arrayDadosAtivos = arrayAtivosExternos.map(row => row.quantidade_total);
  var limiteDados = 7000; // Substitua 5555 pelo elemento que você deseja adicionar

  arrayDadosAtivos.push(limiteDados);

  console.log(arrayDadosAtivos); // A matriz agora incluirá o novo elemento no final
  ...
  var data = {
    ...
    datasets: [{
      ...
      data: arrayDadosAtivos,
    },
    {
      label: 'Qtde de Perda de Ativos',
      borderColor: '#f44336',
      backgroundColor: '#f44336',
      data: arrayAtivosExternos.map(row => row.quantidade_status_9)
    }
    ]
  ...
  var options = {
    plugins: {
      datalabels: {
        ...
        anchor: 'top',
        align: 'top',
        value: arrayAtivosExternos.map(row => row.quantidade_total),
  ...
  const valorFormatTotalVeiculos = document.getElementById('valorFormatTotalVeiculos');
  valorFormatTotalVeiculos.innerHTML = formatoMoeda;
  //console.log(formatoMoeda); // Saída
  ...
  const jsonString = $('#valorTotalVeiculos').val();
  ...
  var limiteDadosVeiculos = '4500000'; 

  arrayDadosVeiculos.push(limiteDadosVeiculos);

console.log(arrayDadosVeiculos);

  // Dados do gráfico
  const dataVeiculos = {
    labels: ["Caminhões", "Máquinas"],
    datasets: [{
      label: ["Veículos"],
      backgroundColor: [
        '#0d6efd',
  ...
  const optionsVeiculos = {
    ...
    plugins: {
      datalabels: {
        ...
        formatter: function (value, context) {         
          return  'R$ ' + value // Exibir o valor do dado diretamente na barra
        }
      }
    },
  };
